...
replace_in_file('src/features/synonyms/components/SynonymNavigationPanel.tsx', [
    ('const { generatePDF, isGenerating: isGeneratingPDF, error: pdfError } = usePDFGenerator(() => import(\'../utils/pdfGenerator\').then(m => m.generateSynonymsPDF as any));', 'const { generatePDF, isGenerating: isGeneratingPDF, error: pdfError } = usePDFGenerator(() => import(\'../utils/pdfGenerator\').then(m => m.generateSynonymsPDF as any));')
])
